...
namespace ConsoleStreamUsing
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            {
                string[] folder1 = Directory.GetDirectories(path);
                if (folder1.Length > 0)
                {
                    foreach (string s1 in folder1)
                    {
                        Console.WriteLine($"{s1}");
                        string[] folder2 = Directory.GetDirectories(s1);
                        if (folder2.Length > 0)
                        {
                            foreach (string s2 in folder2)
                            {
                                Console.WriteLine($"  {s2}");
                                string[] folder3 = Directory.GetDirectories(s2);
                                if (folder3.Length > 0)
                                {
                                    foreach (string s3 in folder3)
                                    {
                                        Console.WriteLine($"   {s3}");
                                    }
                                    Console.WriteLine($"   Найдено папок {folder3.Length}.");
                                }
                            }
                            Console.WriteLine($"  Найдено папок {folder2.Length}.");
                        }
                    }
                    Console.WriteLine($"Найдено папок {folder1.Length}.");
                }
                    ...
                    if (Directory.Exists(dirName))
                    {
                        ...
                        for (byte j = 0; j < 3; j++)
                        {
                            ...
                            if (dirLine < dirNumber)
                            {
                                ...
                                //Начало метода
                                Console.WriteLine("==Сканирование методом Folder()==");
                                Folder(dirs[dirLine]);
                                string line = Console.ReadLine();

                                Console.WriteLine("==Сканирование методом перебора==");
                                if (dirlev1.Length > 0)
                                {
